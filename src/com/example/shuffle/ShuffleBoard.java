...
public class ShuffleBoard extends RelativeLayout {
    ...
    private void flipAcross() {
        ...
        if (currentButton.isSelected()) {
            if (selectedButtons.size() > minButtons) {
                ...
                showToast(getContext(), "不能再少了", Toast.LENGTH_SHORT);
            }
        } else {
    ...
    private void getCurrentXone() {
        ...
        if (pointF.y > unselectedButtonsVertex.y) {
            ...
            if (!currentButton.isSelected() || selectedButtons.size() > minButtons) {
                if (lastZone != crtZone) {
                    if (lastZone == 1) {
            ...
            if (currentButton.isSelected() || selectedButtons.size() < maxButtons) {
                if (lastZone != crtZone) {
                    if (lastZone == 1) {
    ...
    private void putButtonDown() {
        ...
        if (pointF.y > unselectedButtonsVertex.y) {
            ...
            if (!currentButton.isSelected() || selectedButtons.size() > minButtons) {
                ...
                buttons.add(currentButton);
                showToast(getContext(), "不能太少", Toast.LENGTH_SHORT);
            }
            ...
            if (currentButton.isSelected() || selectedButtons.size() < maxButtons) {
                ...
                currentButton.setTargetPosition(new Point(lastCol, lastRow));
                buttons.add(currentButton);
                showToast(getContext(), "不能太多", Toast.LENGTH_SHORT);
            }
        ...
        if (currentButton.isSelected()) {
            ...
            if (buttonCellHeight * selectedButtons.size() / Colums >= minSelectedZoneHeight) {
                ...
                if ((int) (Math.ceil((double) unselectedButtons.size() / Colums))
                        * buttonCellHeight + groupVGap
                        + selectedButtonsVertex.y + selectedButtonsTotalHeight > getHeight()) {
                    if (Math.ceil((double) (selectedButtons.size()) / Colums) * buttonCellHeight > (selectedButtonsTotalHeight - buttonCellHeight)) {
                        return false;
                    } else {
                        return true;
                    }
    ...
    private boolean shouldShrink2() {
        ...
        if (currentButton.isSelected()) {
            if (unselectedButtonsVertex.y + buttonCellHeight
                ...
                if (Math.ceil((double) (selectedButtons.size() - 1) / Colums) * buttonCellHeight > (selectedButtonsTotalHeight - buttonCellHeight)) {
                    return false;
                } else {
                    return true;
                }
