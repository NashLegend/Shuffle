...
public class MainActivity extends Activity {
	...
	private void moveButton(View button, float dx, float dy) {
		...
		slog("the expected pos is " + (button.getX() + dx) + ","
				+ (button.getY() + dy));
		slog("the real pos is " + button.getX() + "," + button.getY());
	...
	private void getCurrentXone() {
		...
		// 一时按钮出去了怎么办。 一旦按钮出去了，则一律按照处在此区的第一个排算……这里的出去指row<0，因为row过大的情况已经考虑了
		//
		slog("xone_" + lastZone + "_" + lastRow + "_" + lastCol);
		PointF pointF = getCurrentButtonCenter();
		slog("the Exact pos is " + pointF.x + "," + pointF.y);
		int crtZone = 0;
		int crtRow = 0;
		int crtCol = 0;
		int aniZone = -1;
	...
	private void putButtonDown() {
		...
		// 上一句作废，我日，up不一定紧跟着move……
		// TODO
		PointF pointF = getCurrentButtonCenter();
		slog("The Last Positon is " + currentButton.getX() + ","
				+ currentButton.getY());
		int crtZone = 0;
		int crtRow = 0;
		int crtCol = 0;
